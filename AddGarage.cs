...
{
    public partial class AddGarage : Form
    {
        public AddGarage()
        {
            InitializeComponent();
            ...
            Sets.SetPercent(textBox12);
            string sql = "SELECT GarageNumber FROM Garages ORDER BY GarageNumber";
            Checks.Max(textBox11, "GarageNumber", sql);
        }
        ...
        {
            if (label12.Text == "תקין" && label13.Text == "תקין" && label14.Text == "תקין" && label15.Text == "תקין" && label16.Text == "תקין" && label17.Text == "תקין" && label18.Text == "תקין" && label19.Text == "תקין" && label20.Text == "תקין" && label21.Text == "תקין" && label24.Text == "תקין" && !string.IsNullOrEmpty(comboBox1.Text) && !string.IsNullOrEmpty(comboBox2.Text))
            {
                ...
                string sql = "SELECT GarageNumber FROM Garages ORDER BY GarageNumber";
                string insert = "INSERT INTO Garages VALUES ('" + Checks.Max(textBox11, "GarageNumber", sql) + "','" + textBox1.Text + "','" + textBox2.Text + "','" + textBox3.Text + "','" + dt1.Rows[0]["ID"].ToString() + "','" + textBox5.Text + "','" + textBox6.Text + "','" + textBox7.Text + "','" + dt2.Rows[0]["Number"].ToString() + "','" + textBox8.Text + "','" + dt3.Rows[0]["Number"].ToString() + "','" + textBox9.Text + "','" + textBox10.Text + "','" + textBox12.Text + "','" + true + "');";
                MyAdoHelperCsharp.DoQuery("Database1.mdf", insert);
                MessageBox.Show("מוסך נוסף בהצלחה");
                UserLogIn.myforms.Remove(UserLogIn.myforms[UserLogIn.myforms.Count - 1]);
                AddCar f = new AddCar("g", textBox11.Text);
                FormInPanel.AddForm(Form1.mainp, f);
                this.Close();
            }
            else
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            if (!Checks.NameLength(textBox1.Text, 4) || !Checks.NameSofiot(textBox1.Text))
            {
                ...
                label12.Text = "שם מוסך זה לא תקין";
            }
            else
            {
        ...
        {
            if (!string.IsNullOrEmpty(textBox6.Text) && MyAdoHelperCsharp.IsExist("Database1.mdf", "SELECT * FROM Garages WHERE GarageCity='" + Checks.GetCityNum(textBox4.Text) + "' AND GarageStreet='" + textBox5.Text + "' AND GarageHNumber !='" + textBox6.Text + "'"))
            {
                DataTable dt = MyAdoHelperCsharp.ExecuteDataTable("Database1.mdf", "SELECT GarageHnumber, GarageZipCode FROM Garages WHERE GarageCity='" + Checks.GetCityNum(textBox4.Text) + "' AND GarageStreet='" + textBox5.Text + "' AND GarageHNumber !='" + textBox6.Text + "'");
                string adress = dt.Rows[0]["GarageHnumber"].ToString();
                string adress2 = textBox6.Text;
                int zipcode = int.Parse(dt.Rows[0]["GarageZipCode"].ToString());
            ...
            {
                if (MyAdoHelperCsharp.IsExist("Database1.mdf", "SELECT * FROM Garages WHERE GarageCity='" + Checks.GetCityNum(textBox4.Text) + "' AND GarageStreet='" + textBox5.Text + "' AND GarageHNumber ='" + textBox6.Text + "'"))
                {
                    DataTable dt = MyAdoHelperCsharp.ExecuteDataTable("Database1.mdf", "SELECT GarageHnumber, GarageZipCode FROM Garages WHERE GarageCity='" + Checks.GetCityNum(textBox4.Text) + "' AND GarageStreet='" + textBox5.Text + "' AND GarageHNumber ='" + textBox6.Text + "'");
                    textBox7.Text = dt.Rows[0]["GarageZipCode"].ToString();
                    textBox7.ReadOnly = true;
                }
                else
                {
        ...
        private void AddGarage_Load(object sender, EventArgs e)
        {
        ...
        {
            if (string.IsNullOrEmpty(textBox12.Text) || int.Parse(textBox12.Text) > 50)
            {
                textBox12.BackColor = Color.Red;
                label24.ForeColor = Color.Red;
                label24.Text = "הנחה זו לא תקינה";
            }
            else
            {
